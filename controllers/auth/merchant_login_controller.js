...
const validatorSchema = require('../../models/merchant_signup_credential');

class Merchant_Login extends BaseModel{
    constructor() {
        super()
    ...
    async signin(user_credentials) {
        try {
            ...
            let signin_data = await this.validateModelSchema(user_credentials, validatorSchema.merchant_signin());
            const last_login_time = moment().utc().toDate();
            const last_login_type = 'normal';
            let collection = this.db.collection(collections.merchant_user);
            let merchant_id= user_credentials.merchant_frysto_id;
            let result = await collection.findOne({
                merchant_frysto_id: signin_data.merchant_frysto_id,
                is_deleted : false
            }, {
                    projection: {
                        "user_id": 1,
            ...
            if (result) {
               ...
               // };
                ...
                console.log(result.merchant_frysto_id);

                if (match) {
                    await collection.updateOne(
                        { 
                            merchant_id: signin_data.merchant_id,
                        },
                        {
                            $set: {
                    ...
                    let jwt_token = await authToken.encrypt({
                        merchant_frysto_id: result.merchant_frysto_id,
                        email: result.email
                    });
